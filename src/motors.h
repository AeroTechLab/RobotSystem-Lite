...
///
/// Interface for configurable motor control. Specific underlying implementation and further configuration are defined as explained in @ref motor_config

/// @page motor_config Motor Configuration
/// The motor-level configuration (see @ref configuration_levels) is read using the [data I/O implementation currently defined](https://bitiquinho.github.io/Platform-Utils/structDataIO.html)
///
...
///     "config": "...",              // Signal input/output device configuration string passed to plugin initialization call
///     "channel": 0                  // Device channel to which output/actuation values will be sent
///   },
///   "output_gain": {              // Signal scaling parameters
///     "multiplier": 1.0,            // Value that multiplies the signal value
///     "divisor": 1.0                // Value that divides the signal value
///   },
///   "output_max": -1.0            // Absolute maximum allowed output value (negative for unlimited)
/// }
/// @endcode
...
/// @brief Verifies if the hardware/virtual device of given motor is output enabled
/// @param[in] motor reference to motor
